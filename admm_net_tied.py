...
class ADMMNet(tf.keras.Model):
  ...
  def __init__(self, p, num_stages, *args, **kwargs):
    ...
    self.rho0 = self.params_init['rho']
    self.alpha0 = self.params_init['alpha']
    self.lambda0 = self.params_init['lambda']
  ...
  def call(self, inputs):
    # layer 1
    z_0 = np.zeros((2*self.n,1),dtype=np.float32)
    u_0 = np.zeros((2*self.n,1),dtype=np.float32)
    z,u = self.Layers[0](inputs, z_0, u_0, self.M)
    
    # layers 2, 3, ... 
    for l in self.Layers[1:]:
      z,u = l(inputs,z,u, self.M)
...
class Stage(layers.Layer):
  ...
  def call(self, y, z, u, M):
    M1, M2 = M

    x = tf.matmul(M1, tf.transpose(y)) + tf.matmul(M2, (z-u))
